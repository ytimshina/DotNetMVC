// js/controllers/mainControllerDynamic.js
// Main controller with complete dynamic fan data system - NO STATIC DATA

import { ERVCalculatorCompleteExact } from '../modules/ervCalculatorCompleteExact.js';

export class MainControllerDynamic {
    constructor() {
        this.calculator = new ERVCalculatorCompleteExact();
        this.isCalculating = false;
        this.isInitialized = false;

        // Cell mapping based on Excel analysis (same structure, dynamic calculations)
        this.cellMapping = {
            inputs: {
    ...
    async init() {
        try {
            console.log('Initializing MainControllerDynamic with complete dynamic fan data system...');
            ...
            console.log('Ready for user input - complete dynamic system loaded');

            this.isInitialized = true;
            console.log('MainControllerDynamic initialized successfully');

        } catch (error) {
            console.error('Failed to initialize MainControllerDynamic:', error);
            throw error;
        }
    }
    ...
    async initializeDropdowns() {
        ...
        // Unit Models (exact from Excel) - CRITICAL for dynamic fan calculation
        const unitModels = [
            ...
            { value: "Trane - Precedent E", text: "Trane - Precedent E" }, // Important for P77 vs N77 logic
    ...
    setupEventListeners() {
        ...
        // Add special listener for unit model changes (affects fan calculation)
        const unitModelSelect = document.getElementById('C21');
        if (unitModelSelect) {
            unitModelSelect.addEventListener('change', () => {
                console.log('Unit model changed to:', unitModelSelect.value);
                this.checkAndCalculateIfReady();
            });
        }

        // Add special listener for CFM changes (affects fan calculation)
        const cfmInputs = ['C15', 'C16', 'C17'];
        cfmInputs.forEach(inputId => {
            const input = document.getElementById(inputId);
            if (input) {
                input.addEventListener('input', () => {
                    console.log(`${inputId} changed to:`, input.value);
                    this.checkAndCalculateIfReady();
                });
            }
        });
    ...
    collectInputs() {
        ...
        // Flow inputs - CRITICAL for dynamic fan calculation
        ...
        // Equipment inputs - CRITICAL for dynamic fan calculation
    ...
    validateInputs(inputs) {
        ...
        // Validate inputs for dynamic fan calculation
        if (inputs.unitModel && inputs.outdoorAirCFM) {
            const cfm = parseFloat(inputs.outdoorAirCFM);
            if (cfm < 500 || cfm > 10000) {
                errors.push('Outdoor Air CFM must be between 500 and 10,000 for fan calculation');
            }
        }
    ...
    async performCalculation() {
        ...
        try {
            ...
            console.log('Starting dynamic calculation with inputs:', {
                unitModel: inputs.unitModel,
                outdoorCFM: inputs.outdoorAirCFM,
                supplyAirCFM: inputs.supplyAirCFM,
                method: 'Complete dynamic Excel-exact system'
            });

            // Perform calculation using the complete dynamic system
            await this.calculator.calculate(inputs);

            // Display results including dynamic H6-H10 calculations
            this.displayResults();
            this.showResultsWidget();

            console.log('Complete dynamic Excel-exact calculation completed successfully');
    ...
    clearResults() {
        // Clear all calculated fields including H6-H10
        const calculatedFields = [
            'C11', 'C12', 'C18', 'D11', 'D12', 'D14', 'D26', 'D27',
            'H6', 'H7', 'H8', 'H9', 'H10', // Fan data fields
            'G16', 'H16', 'G17', 'H17', 'G18', 'H18', 'G19', 'H19', 'H20',
            'I26', 'I27',
    ...
    displayResults() {
        ...
        console.log('Displaying results with dynamic H6-H10 calculations:', {
            fanType: results.fanType,
            motorHP: results.motorSizeHP,
            fanRPM: results.fanRPM,
            fanBHP: results.fanMotorBHP,
            staticPressure: results.totalStaticPressure,
            calculationMethod: results.calculationMethod,
            fanCalculationPath: results.fanCalculationPath
        });

        // Update calculated fields including dynamic fan data
        const calculatedFields = {
            // Grains calculations
            ...
            // DYNAMIC Fan Data (H6-H10 - calculated in real-time)
            'H6': results.fanType || '--',
            ...
            // DYNAMIC Motor, Belt and Pulleys (calculated from Parts sheet)
        ...
        const winterResults = {
            ...
            'D35': results.pressureDropCooling || '--', // Same as cooling
        ...
        // Log dynamic calculation details for validation
        if (results.fanCalculationPath) {
            console.log('Dynamic fan calculation path:', results.fanCalculationPath);
        }
        if (results.fanValidation) {
            console.log('Fan calculation validation:', results.fanValidation);
        }
    }
    ...
    async saveCalculation() {
        try {
            ...
            const data = {
                ...
                calculationMethod: 'Complete dynamic Excel-exact with real-time fan calculations',
                version: '3.8.0-complete-dynamic',
                fanCalculationDetails: results.fanCalculationPath
            };
    ...
    exportResults() {
        ...
        const exportData = {
            ...
            calculationMethod: 'Complete dynamic Excel-exact with real-time fan calculations',
            fanCalculationDetails: results.fanCalculationPath,
            fanValidation: results.fanValidation
        };
        ...
        a.href = url;
        a.download = `erv-calculation-dynamic-${new Date().toISOString().split('T')[0]}.json`;
